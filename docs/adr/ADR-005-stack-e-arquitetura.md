...
> **Limite honesto:** sem (A)/(B), o churn de 2 conexões por request continua — mas a **auto-recuperação da tela** (acima) faz o soluço se curar sozinho em segundos, e o **throttle (C)** reduz a frequência da rajada. A solução definitiva segue sendo **Supabase Pro** (sem auto-pause, pool estável) + Workers **pago** (CPU 30 s vs. 10 ms) no lançamento.

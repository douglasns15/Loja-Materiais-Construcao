...
>
> **(A) — o que fica de pé:** a redução real do churn é **uma instância por REQUEST**, compartilhada entre `requireAuth` e o handler via **contexto do Hono** (`c.set('prisma')`) — segura por ser DENTRO da mesma request. Exige tocar os handlers para lerem do contexto em vez de chamar `createPrismaClient` de novo; **fica para uma fatia futura**.

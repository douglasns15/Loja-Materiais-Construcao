...
- **(B) reuso do `PrismaClient` entre requests do isolate** — **TENTADO E REVERTIDO** (ver abaixo).
- **(C) Throttle do sino** (`apps/web/.../AlertsChip.tsx`): o refetch de `/alerts` por **foco** da janela só dispara se passou **≥60 s** do último — corta a rajada repetida a cada alt-tab. **Vale.**

> **❌ A parte (B) quebrou em produção (2026-09-04).** A ideia era `createPrismaClient` **cachear o `PrismaClient` por connection string** (`Map` vivo pelo isolate) para `requireAuth` e o handler compartilharem e requests seguintes reusarem. Foi ao ar (API `c0705cb8`) e o Owner bateu no erro **logo na 1ª venda de teste**. Causa: o **Cloudflare Workers proíbe usar um objeto de I/O** (o socket do `pg.Pool`, aberto no contexto de UMA request) **em OUTRA request** → 500 *"Cannot perform I/O on behalf of a different request"*. Medição no `/db-check` (público): **~50% das requisições falhando** (alternando 1 falha / 1 ok) — **muito pior** que o soluço intermitente original. **Revertido** para criação por chamada (API `76367777`); stress test pós-revert = **0 falhas em 15 sequenciais + 10 paralelas**. Commit do revert `392d081`. Lição: o reuso de conexão entre requests **não** é seguro no par Workers + `pg`/Hyperdrive; o padrão é criar por request.
>
> **✅ (A) "Conexão única por requisição" — IMPLEMENTADA (2026-09-04):** a redução real e **segura** do churn. Novo helper `getPrisma(c)` (`apps/api/src/lib/request.ts`) cria o `PrismaClient` **uma vez por requisição** e o memoiza no **contexto do Hono** (`c.set('prisma')`); `requireAuth` e os ~90 handlers passaram a chamar `getPrisma(c)` em vez de `createPrismaClient(connectionString)` — **2 → 1 conexão por request** (e um pool "vazado" a menos por request, e uma instanciação de Prisma a menos = menos CPU). Seguro porque o client nasce e morre DENTRO da mesma request — NUNCA cruza requests (foi isso que derrubou a parte B). `createPrismaClient` segue para o keep-alive (handler `scheduled`, sem contexto Hono), o `/db-check` e os helpers tipados com `ReturnType<typeof createPrismaClient>`.

> **✅ E2E do Owner VALIDADO (2026-09-04):** vendas concluídas com sucesso no ambiente de testes — confirma o `getPrisma(c)` sob uso autenticado real (a rota da venda passa pelo `requireAuth` + handler compartilhando o mesmo client). **NO AR:** API `c918e8e4` + web `9b3c36ca`; commits `da6fa48` + `392d081` + `f1be37d` + `662ac7d` em `main` (push do Owner).
>
> **Limite honesto:** com (A) o churn caiu para **1 conexão por request** e o **throttle (C)** reduz a frequência da rajada; a **auto-recuperação da tela** faz o soluço que ainda escapar se curar sozinho em segundos (antes: minutos). Mas o **teto de conexões/CPU do free tier continua lá** — a solução definitiva segue sendo **Supabase Pro** (sem auto-pause, pool estável) + Workers **pago** (CPU 30 s vs. 10 ms) no lançamento.

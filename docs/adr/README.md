...
| [ADR-032](./ADR-032-atalhos-de-teclado.md) | Atalhos de teclado configuráveis e cross-platform (motor por `event.code`; F-keys p/ ações quentes + sequência `N`+letra p/ navegar; ajuda `?`; tela Configurações → Acessibilidade; `localStorage` custo-zero; desktop-only) | Aceito — Fatia 1 (motor + navegação) em implementação; sem migração |

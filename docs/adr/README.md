...
| [ADR-029](./ADR-029-central-de-pendencias-computada.md) | Central de pendências computada (sino de alertas de cadastro/estoque/financeiro calculados sob demanda; sem push, sem tabela, sem migração; nasce visível a todos, role-scoping depois) | Aceito — implementado/validado (2026-09-02); detalhado no [plano da Central de Alertas](../plano-central-de-alertas.md) |
| [ADR-030](./ADR-030-pacote-como-unidade-fechada.md) | Pacote como unidade fechada (generaliza o ADR-017: pacote fechado como principal + venda por unidade avulsa opcional; régua fina = unidade com passo 1; estoque "X pacotes + Y un") | Aceito — implementado 2026-09-04, aguardando deploy + E2E do Owner; migration `0037` (enum `PACK`) |
| [ADR-031](./ADR-031-janela-flutuante-de-tela.md) | Janela flutuante de tela (painel destacável e interativo no mesmo documento — análise comparativa; reusa o componente de página; leitura/busca; desktop-only; custo-zero. `window.open`/Document PiP deferidos) | Aceito — Fatia 1 implementada, NO AR e E2E do Owner validado em produção (2026-09-05); sem migração |
| [ADR-032](./ADR-032-atalhos-de-teclado.md) | Atalhos de teclado configuráveis e cross-platform (motor por `event.code`; F-keys p/ ações quentes + sequência `N`+letra p/ navegar; ajuda `?`; tela Configurações → Acessibilidade; `localStorage` custo-zero; desktop-only) | Aceito — Fatias 1–4 implementadas, NO AR e E2E do Owner validado (2026-09-05); sem migração |
| [ADR-033](./ADR-033-devolucao-unificada-defeito-estorno-troca.md) | Devolução unificada: condição do item (defeito não volta ao estoque + lista/resolução com fornecedor), forma do estorno explícita no caixa (*same-tender refund*), troca integrada ao PDV e unificação dos botões Cancelar/Devolver do Histórico | Aceito — Fatias 1 (defeito, `0038`), 2 (UI unificada + forma do estorno, `0039`) e 3 (troca no PDV, `0040`) **NO AR** (API `24ab85d2`, web `85eb33b5`); E2E do Owner pendente (sessão à parte) |
| [ADR-034](./ADR-034-cliente-opcional-em-venda.md) | Cliente opcional em qualquer venda (identidade ortogonal ao pagamento; anexar cliente também em venda paga à vista/cartão/PIX; remove acoplamento de UI que só anexava via fiado/crédito/retirada — sem migration, sem mudança de contrato; destrava "Crédito na loja" no retorno, achado #1 do E2E da ADR-033) | Aceito — NO AR + E2E do Owner validado (2026-09-10); sem migration |
| [ADR-037](./ADR-037-recebido-liquido-de-estornos.md) | "Recebido" líquido de estornos: dinheiro que entrou − dinheiro que voltou ao cliente (gaveta ou estorno cartão/PIX, rateado por forma, abatido no dia da venda); crédito na loja e abatimento de dívida não abatem (informativos); gráfico diário e detalhe por forma coerentes; lucro/margem/rankings líquidos do devolvido/trocado. Revisa ADR-036 A/B para o Recebido. Sem migration | Aceito — implementado 2026-09-25; deploy/E2E/push pendentes |
| [ADR-036](./ADR-036-relatorios-coerentes-devolucoes-trocas-credito.md) | Relatórios coerentes com devoluções/trocas/crédito: venda devolvida por inteiro sai do faturamento (status `RETURNED`); "Recebido" conta só dinheiro real (crédito da loja e vale-troca fora); líquido subtrai só devoluções parciais; contadores de Trocas/Devolvidas; + card do Histórico refletindo o desfecho (Cancelada/Devolvida/Trocada→V-XXX/Devolução parcial + valor ajustado). Sem migration | Aceito — NO AR + E2E do Owner validado (2026-09-10) |
| [ADR-035](./ADR-035-credito-no-retorno-pick-cliente-e-faturamento-liquido.md) | Crédito no retorno com cliente escolhido no ato (*pick-no-retorno*: seleciona/cadastra o cliente ao dar "Crédito na loja" e o anexa à venda) + botão único **"Cancelar / Devolver"** com passo de intenção de 3 caminhos (Devolveu/desistiu · Trocar · Cancelar por erro — fiscal-ready p/ NF-e: nota de devolução × nota de cancelamento) + faturamento líquido de devoluções (bruto e líquido) nos relatórios | Aceito — NO AR + E2E do Owner validado (2026-09-10); achados de relatório originaram a ADR-036; sem migration |

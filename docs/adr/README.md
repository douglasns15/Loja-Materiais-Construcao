...
| [ADR-037](./ADR-037-emissao-fiscal-nfce.md) | Emissão fiscal de NFC-e: provedor (Focus NFe), guarda do certificado, numeração e contingência | **Proposto** — Fatia 0 (domínio + porta + adaptador de simulação) implementada em `apps/fiscal`; decisões aguardando aprovação |
| [ADR-038](./ADR-038-empresa-acima-da-loja.md) | Empresa (grupo) acima da Loja: o que `Tenant` modela e quando separar | Aceito — decisão de **adiar** a separação, com invariantes fixadas; sem mudança de schema |

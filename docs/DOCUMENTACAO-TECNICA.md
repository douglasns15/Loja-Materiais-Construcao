...
| `GET /top-products` | Ranking de produtos no período (`?q=&orderBy=faturamento\|lucro&from=&to=&limit=`): faturamento, qtd, nº de vendas e lucro/margem (custo carimbado, ADR-027) com cobertura. Busca sem acento (Fatia 5). **ADR-037:** faturamento/qtd/custo **líquidos do devolvido/trocado**; linha devolvida por inteiro não conta como venda; produto devolvido por inteiro sai do ranking (vale também para `/product-customers`, `/top-customers` e `/customer-products`). |

...
| `POST /:id/return-items` 🔒 | Devolução **parcial** por item; excedente vira crédito na loja, dinheiro, ou **estorno na mesma forma** (`target` = `STORE_CREDIT`/`CASH`/`SAME_AS_PAYMENT` — ADR-033; no estorno só a parcela em dinheiro sai do caixa). Cada item aceita `condition` (`GOOD` padrão × `DEFECTIVE`): defeito **não** volta ao estoque vendável — vira defeituoso (`Product.defectiveQty` + ledger na linha). `intent = EXCHANGE` (ADR-033): é uma **troca** — o valor não vira dinheiro/crédito, vira um vale (a nova venda o consome via `exchangeReturnId`). |

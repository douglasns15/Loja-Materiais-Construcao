...
| Método · Rota | O que faz |
|---|---|
| `GET /` | Lista vendas (paginação por cursor + filtro de período). Busca no Histórico (`?scope=all`, ignora o período): `number` (código V-000128), `customer` (nome do cliente — só casa vendas com cliente vinculado) e `value` (total exato). As linhas incluem o nome do cliente quando há. **Resumo de desfecho (ADR-036):** cada linha traz `returnedValue` (Σ devolvido em REFUND), `exchangedValue` (Σ trocado) e `exchangedTo` (nº das vendas geradas na troca) para o card refletir Devolvida/Devolução parcial/Trocada. |
| `POST /` 🔒 | Confirma a venda: transação `Order` + `OrderItem` + `Payment` + `StockMovement` (ADR-001). `items[].discount` (ADR-036): desconto **por item** (R$ da linha) — `total = qtd × preço − desconto`. **Troca ATÔMICA** (`exchangeReturn` — ADR-033 revisado): `{ fromOrderId, reason, items[] }` executa a **devolução da venda de origem + esta compra na MESMA transação** — valida as quantidades devolvíveis, calcula o vale (valor pago líquido de descontos), estorna o estoque, cria a devolução `EXCHANGE` já amarrada a esta venda (`exchangeOrderId`) e grava a parcela `EXCHANGE_CREDIT` (não toca o caixa); exige total ≥ vale; online-only. Substitui o antigo `exchangeReturnId` (o vale não é mais pré-gravado, então cancelar/atualizar no PDV não deixa devolução órfã nem devolve estoque). **Custo carimbado (ADR-027, fix 2026-09-25):** `OrderItem.unitCost` é por **unidade-base** via `costPerBaseUnit` (core) — em unidade fechada (barra/rolo/pacote) o `costPrice` do cadastro é do inteiro, então grava `costPrice ÷ tamanho` (antes gravava o custo da barra e o lucro saía multiplicado pelo tamanho). |
| `POST /:id/cancel` 🔒 | Cancela venda do caixa aberto (estorna estoque; sai do faturamento). Aceita `refundMethod` (`SAME_AS_PAYMENT` padrão × `CASH`): em `CASH` lança a saída da parte que não era dinheiro (ADR-033). `items[].condition` marca defeito (imediata): não repõe, entra na fila de defeituosos. Bloqueia se a venda já teve devolução parcial. |
...
| Método · Rota | O que faz |
|---|---|
| `GET /defective` 🔒 | Fila de itens devolvidos com **defeito** ainda pendentes (produto, qtd parada, valor, motivo, venda de origem, fornecedor candidato à troca). |
| `POST /defective/:id/resolve` 🔒 | Resolve um defeito: `RESTOCK` (fornecedor trocou → repõe no estoque vendável) ou `WRITE_OFF` (baixa/perda). Sai do `defectiveQty`; transação atômica + auditoria. |
...
| Método · Rota | O que faz |
|---|---|
| `GET /sales` | Vendas por período (faturamento CONFIRMED, canceladas à parte, total por forma de pagamento) + **lucro bruto/margem estimados** (base de mercadoria vendida, custo carimbado ADR-027, com cobertura) — Fatia 6. Com `?compare=1` (e intervalo), inclui `previous` = KPIs da janela ANTERIOR equivalente para os selos ▲/▼ (Fatia 4). **ADR-037:** `totalRevenue` ("Recebido") é **líquido de estornos** = `grossRevenue` (entradas: pagamentos em dinheiro real das vendas do período + recebimentos de fiado) − `returnsTotal` (dinheiro que voltou ao cliente nas devoluções REFUND dessas vendas — gaveta ou estorno cartão/PIX, rateado por forma e atribuído ao **dia da venda**); `byPaymentMethod` já vem líquido por forma (uma forma pode ficar negativa, ex.: cartão estornado em dinheiro); `netRevenue` = `totalRevenue` (compat.); novos `grossRevenue`, `returnsToCredit` (virou crédito na loja — não sai do Recebido) e `returnsToDebt` (abateu dívida — informativo). Vendas devolvidas por inteiro pela devolução por item contam o pago e abatem o estorno (a devolvida em crédito mantém o dinheiro); só a devolução total da rota antiga `/return` segue fora. `goodsRevenue`/lucro/margem descontam a fração devolvida/trocada de cada item (`returnedBaseQty`); `creditSalesGenerated` é líquido do abatido por devolução. |
| `GET /payment-composition` | Drill-down de UMA forma (`?method=&from=&to=`): as linhas (venda à vista + recebimento de dívida + **estorno de devolução**, `tipo: 'estorno'` com `valor` negativo e data da venda — ADR-037) que somam o "Recebido" da forma — Σ linhas = total daquela forma no `/sales` (Fatia 3, Relatórios v2). |
| `GET /top-products` | Ranking de produtos no período (`?q=&orderBy=faturamento\|lucro\|quantidade&from=&to=&limit=`): faturamento, qtd, nº de vendas e lucro/margem (custo carimbado, ADR-027) com cobertura. Busca sem acento (Fatia 5). **Mais vendidos (2026-09-25):** `orderBy=quantidade` ordena por `baseQty` (quantidade em **unidade-base** do produto — `baseQuantity ?? quantity` − devolvido, a mesma do estoque; desempate por faturamento); toda linha passa a trazer `baseQty` + `unit` (unidade do cadastro, `null` se o produto saiu do cadastro) + `closedSize` (tamanho da barra/rolo/pacote quando o produto é unidade fechada — ADR-017/030 —, pois aí `baseQty` está na régua fina, metro/unidade avulsa; o front mostra "2 barras + 3 m"; `null` nos demais). `qty` segue na unidade vendida (pode misturar embalagem e base). `/top-customers` não aceita `quantidade`. **ADR-037:** faturamento/qtd/custo **líquidos do devolvido/trocado**; linha devolvida por inteiro não conta como venda; produto devolvido por inteiro sai do ranking (vale também para `/product-customers`, `/top-customers` e `/customer-products`). |
| `GET /product-customers/:productId` | "Quem mais compra" um produto (`?from=&to=`): top clientes por faturamento naquele produto — detalhe do pop-up (Fatia 5). Traz também `baseQty` (unidade-base, líquida — 2026-09-25), rotulada no front com `unit`/`closedSize` do produto. |
...
| `GET /customer-products/:customerId` | "O que costuma comprar" um cliente (`?from=&to=`): top produtos por faturamento daquele cliente — detalhe do pop-up (Fatia 5). Traz também `baseQty` + `unit` + `closedSize` (2026-09-25), como o `/top-products`. |
| `GET /daily` | Recebido por DIA no período (`?from=&to=`) para o gráfico de barras (Fatia 7): à vista pela data da venda + fiado por `paidAt`, por dia (fuso −3h). Σ dias = "Recebido do período". **ADR-037:** exclui crédito da loja/vale-troca (antes somava e não batia com o card) e abate os estornos no dia da venda, por forma. |

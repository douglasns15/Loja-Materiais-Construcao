...
| `GET /projections` | Projeções "no ritmo atual" (Fatia 8), independentes do filtro: faturamento do mês (run-rate), a receber próx. 30 dias (vencimentos ADR-026) e itens que vão faltar (MEDIANA do consumo diário — robusta a pico único —, StockMovement EXPENSE 30d). |

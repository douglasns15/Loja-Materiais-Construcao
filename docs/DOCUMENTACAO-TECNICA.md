...
| `POST /` 🔒 | Confirma a venda: transação `Order` + `OrderItem` + `Payment` + `StockMovement` (ADR-001). Aceita `exchangeReturnId` (ADR-033): consome o vale de uma devolução `EXCHANGE` — grava a parcela `EXCHANGE_CREDIT` (não toca o caixa) e amarra a devolução à venda; exige total ≥ vale. |
